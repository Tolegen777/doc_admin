...
export const TimeSlotContent = memo(({ timeSlot }: Props) => {
  ...
  const handleChangeTimeSlot = (item: WorkingHoursList) => {
    ...
    const index = newState.indexOf(item.time_slot_object_id);
    if (index !== -1) {
      newState.splice(index, 1);
    } else if (!item.reserved) {
      newState.push(item.time_slot_object_id);
    }
    dispatch({
    ...
    <div
      className={clsx({
        ...
        [styles.container_active]:
          selectedTimeSlotIds?.includes(timeSlot?.time_slot_object_id) &&
          timeSlot?.panel_colour !== "grey",
      ...
      data-id={!timeSlot?.reserved ? timeSlot?.time_slot_object_id : null}
    >
      {removeSeconds(timeSlot?.time_slot_object_start_time)}
    </div>
  );
});
