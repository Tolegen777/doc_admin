...
export const SlotEditBlockContent = memo(({ workingHours }: Props) => {
  ...
  const borderSelectionContainer = document.getElementById(
    "root",
  ) as HTMLElement;
  ...
  const formatWorkingHours = () => {
    return workingHours.map((item) => ({
      ...
      isActive:
        !item.reserved &&
        selectedTimeSlotIds?.includes(item.time_slot_object_id),
    }));
  };

  const memoizedTimeSlots = useMemo(formatWorkingHours, [selectedTimeSlotIds]);
  ...
  return (
    <div className={styles.container}>
      <main className={styles.container_content} ref={containerRef}>
        {memoizedTimeSlots?.map((item) => (
          <TimeSlotContent key={item?.time_slot_object_id} timeSlot={item} />
        ))}
      </main>
      <ReactMouseSelect
