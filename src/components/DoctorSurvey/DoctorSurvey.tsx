...
import { IAllCategories } from "../../types/categories.ts";
  ...
  const { mutate: onUpdate, isPending: isUpdateLoading } = useMutation({
    ...
    mutationFn: (body: ICreateUpdateDoctor) => {
      return axiosInstance.patch(
        `employee_endpoints/doctors/${pathname?.id}`,
        body,
      );
    },
    onSuccess: () => {
  ...
  const { data, isLoading: categoryLoading } = useQuery({
    ...
    queryFn: () =>
      axiosInstance
        .get<
          IAllCategories[]
        >(`employee_endpoints/doctors/list_of_all_medical_categories/`)
        .then((response) => {
  ...
  const { isLoading } = useQuery({
    ...
    queryFn: () =>
      axiosInstance
        ...
        .then((response) => {
          if (response) {
            const fullNameObject = splitFullName(response.data.full_name);

            setCreateUpdateFormInitialFields(
              changeFormFieldsData(initialValues, {
                ...response?.data,
                ...fullNameObject,
                description: response.data.description_fragments[0].content,
                works_since: datePickerFormatter(
                  response?.data?.works_since ?? "",
                ),
              }),
            );
          }
