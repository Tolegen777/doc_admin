...
export const DoctorWorkScheduleCreateUpdateForm = (props: Props) => {
    ...
    const options = selectOptionsParser(clinics?.results ?? [], 'title', 'id')
    const workingHoursOptions = selectOptionsParser(workingHours ?? [], 'start_time', 'time_slot_id')
    ...
    const formFields = [
        ...
        {
            name: 'work_date',
            element: <DatePicker placeholder="Выберите дату" style={{ width: '100%' }} />,
            label: 'Дата работы',
            rules: [{
                required: true,
                message: 'Обязательное поле!'
