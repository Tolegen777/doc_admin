...
const ReviewsPage = () => {
  ...
  const { mutate: onUpdate, isPending: isUpdateLoading } = useMutation({
    ...
    mutationFn: ({ id, ...body }: IReviewPayload) => {
      return axiosInstance.patch(
        `patients_endpoints/clinics/${addressSlug}/reviews/manage/${id}/`,
        body,
      );
    },
    onSuccess: () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["reviewsData", addressSlug, page],
    queryFn: () =>
      axiosInstance
        .get<
          IGet<IReview>
        >(`patients_endpoints/clinics/${addressSlug}/reviews/?page=${page}`)
        .then((response) => response?.data),
    enabled: Boolean(addressSlug),
  });
  ...
  const columns = [
    ...
    },
    {
      title: "Процедура",
      key: "procedure_title",
      render: (review: IReview) => <div>{review?.procedure_title}</div>,
    },
    {
      title: "Пациент",
      key: "patient_name",
    ...
    },
    {
      title: "Текст отзыва",
      key: "text",
      render: (review: IReview) => <div>{review?.text}</div>,
    },
    {
      title: "Рейтинг",
      key: "rating",
      render: (review: IReview) => <div>{review?.rating}</div>,
    },
    {
